...
clustering_function = load(f'{environment}_k_means_scale_{scaler_name}_{num_clusters}_{num_traces}')
scaler = load(f"{scaler_name}_{environment}_{num_traces}")
...
crashes, nr_tries, nr_reaches_actual = prob_reach_checking(env, scaler, clustering_function, n_traces_per_ref,
                                                           ...
                                                           mdp=mdp, refine_mdp=True, include_reward=True)
