...
def prob_reach_checking(env, scaler, ae, clustering_function, n_traces_per_ref,
    ...
    for i in range(model_refinements):
        ...
        for j in range(n_traces_per_ref):
            ...
            if scaler and not ae:
                conc_obs = scaler.transform(conc_obs)
            if ae:
                #conc_obs = conc_obs[0].astype(np.double)
                #print(conc_obs)
                conc_obs = ae.encoder(torch.tensor(conc_obs)).detach().numpy().reshape(1, -1).astype(float)
